	...
	@section('content')
		<div class="card">
			<div class="card-heading">
				<span class="card-title">Edit Metric</span>
			</div>
			<div class="card-body">
				<form class="form-horizontal" method="post" action="{{ route('metrics.update', $metric->id) }}">
					{!!csrf_field()!!}
					<div class="col-sm-6">
						<div class="md-form-group {{$errors->has('metric_name') ? 'has-error' : ''}}">
				          <input class="md-input" name="metric_name" value="{{old('metric_name', $metric->metric_name)}}">
				          <label>Metric</label>
					...
					<div class="col-sm-6">
						<div class="md-form-group {{$errors->has('score_index') ? 'has-error' : ''}}">
				          <input class="md-input" name="score_index" value="{{old('score_index', $metric->score_index)}}">
					...
					<div class="col-sm-12">
				        <div class="md-form-group {{$errors->has('details') ? 'has-error' : ''}}">
				        	<textarea class="md-input" name="details" rows="4">{{old('details', $metric->details)}}</textarea>
