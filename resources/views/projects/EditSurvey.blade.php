	...
	@section('content')
		<div class="card">
			<div class="card-heading">
				<span class="card-title">Update Survey Information</span>
			</div>
			<div class="card-body">
				<form class="form-horizontal" method="post" action="{{ route('projects.update', $project->id) }}" enctype="multipart/form-data">
					...
					<div class="md-form-group {{$errors->has('project_name') ? 'has-error' : ''}}">
			          <input class="md-input" name="project_name" value="{{old('project_name', $project->project_name)}}">
			          <label>Title</label>
			          {!!$errors->first('project_name', '<span class="help-block">:message</span>')!!}
			        </div>
			        <div class="md-form-group">
			        	<textarea class="md-input" name="details" rows="4">{{old('details', $project->details)}}</textarea>
			          <label>Details</label>
			        </div>
			        <div class="md-form-group">
